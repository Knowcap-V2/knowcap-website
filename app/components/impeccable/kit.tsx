 ...
 * Type vars (--ve-display / --ve-body / --ve-mono) are set route-side via
 * next/font and passed in as `fontVars` on the page root.
 */
...
export const APP_URL = 'https://app.knowcap.ai'

/* ----------------------------------------------------------------------- */
/* Scoped styles (shared with the impeccable visual system)                 */
/* ----------------------------------------------------------------------- */

export const IMPECCABLE_CSS = `
.ve-root{
...
.ve-hero h1{margin:0 0 26px;max-width:18ch}
.ve-hero .ve-lead{color:var(--ink-prose-dim);max-width:56ch;margin:0 0 38px}
.ve-cta-row{display:flex;flex-wrap:wrap;gap:14px;margin-bottom:46px}
.ve-bullets{display:flex;flex-direction:column;gap:14px;border-top:1px solid var(--line-dark);padding-top:28px;max-width:680px}
...
/* split exhibit (narrative + framed media, for signature sections) */
.ve-split{display:grid;grid-template-columns:1fr;gap:clamp(28px,4vw,64px);align-items:center}
@media(min-width:880px){.ve-split{grid-template-columns:1fr 1fr}.ve-split--flip .ve-split-media{order:-1}}
.ve-split-rule{width:34px;height:2px;background:var(--brass);margin-bottom:20px}
.ve-split h3{margin:0 0 16px}
.ve-split p{margin:0 0 14px;color:var(--prose-dim)}
.ve-split p:last-child{margin-bottom:0}
.ve-figure-xl{font-family:var(--fmono);font-weight:600;letter-spacing:-.02em;line-height:1;
  color:var(--brass-ink);font-size:clamp(2.4rem,5vw,3.4rem);margin:0 0 18px;display:block}

/* process / numbered sequence with drawn rail (also used for escalation levels) */
.ve-process{display:grid;grid-template-columns:1fr;gap:0;position:relative;max-width:820px}
...
.ve-step-no{font-family:var(--fmono);font-weight:600;font-size:1rem;color:var(--brass);
  letter-spacing:.04em;padding-top:.18em;position:relative;white-space:nowrap}
...
.ve-step .ve-step-tag{font-family:var(--fmono);font-size:.7rem;letter-spacing:.1em;text-transform:uppercase;
  color:var(--brass);display:block;margin-bottom:6px}
.ve-step p{margin:0;color:var(--ink-prose-dim);max-width:62ch}
.ve-process-cta{margin-top:clamp(40px,5vw,60px)}
.ve-process-note{margin-top:clamp(28px,4vw,40px);text-align:center;font-family:var(--fmono);
  font-size:.72rem;letter-spacing:.06em;color:var(--ink-prose-dim)}
...
/* ----------------------------------------------------------------------- */
/* Small inline glyphs (custom — no icon library)                           */
/* ----------------------------------------------------------------------- */
...
/* ----------------------------------------------------------------------- */
/* Shared chrome — identical across all impeccable variants                 */
/* ----------------------------------------------------------------------- */

export function ImpeccableHeader() {
...
export function ImpeccableFooter() {
...
export function ImpeccableFloatingCTA() {
...
/* ----------------------------------------------------------------------- */
/* Shared body sections — the CONSTANT across the B/D copy test             */
/* (identical content + design in both variants; only hero + signature     */
/*  section differ between B-imp and D-imp)                                  */
/* ----------------------------------------------------------------------- */
...
export function SecuritySection() {
  return (
    <section id="security" className="ve-section ve-dark ve-pad">
      <div className="ve-field" aria-hidden="true" />
      <div className="ve-wrap" style={{ position: 'relative' }}>
        <SectionReveal className="ve-head">
...
export function CloseSection({ title, sub }: { title: React.ReactNode; sub: string }) {
  return (
    <section className="ve-section ve-dark ve-pad">
      <div className="ve-field" aria-hidden="true" />
      <div className="ve-wrap" style={{ position: 'relative' }}>
        <SectionReveal className="ve-close">
          <h2 className="ve-h2">{title}</h2>
          <p>{sub}</p>
